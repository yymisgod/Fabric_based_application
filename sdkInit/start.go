...
const (
	...
	//ChannelConfigFile = "/opt/gopath/src/github.com/kongyixueyuan.com/modelworker/fixtures/artifacts/channel.tx"
...
func CreateChannel(sdk *fabsdk.FabricSDK, info *InitInfo) error {
	...
	if err != nil {
		...
		// 	err = resMgmtClient.SaveChannel(channelReq, resmgmt.WithRetry(retry.DefaultResMgmtOpts), resmgmt.WithOrdererEndpoint(Orderer2OrgName))
		// 	if err != nil {
		// 		err = resMgmtClient.SaveChannel(channelReq, resmgmt.WithRetry(retry.DefaultResMgmtOpts), resmgmt.WithOrdererEndpoint(Orderer3OrgName))
		// 		if err != nil {
		// 			return fmt.Errorf("创建应用通道失败: %v", err)
		// 		}
		// 	}
	...
	if err != nil {
		...
		// err = OrgResMgmt.JoinChannel(ChannelID, resmgmt.WithRetry(retry.DefaultResMgmtOpts), resmgmt.WithOrdererEndpoint(Orderer2OrgName))
		// if err != nil {
		// 	err = OrgResMgmt.JoinChannel(ChannelID, resmgmt.WithRetry(retry.DefaultResMgmtOpts), resmgmt.WithOrdererEndpoint(Orderer3OrgName))
		// 	if err != nil {
		// 		return fmt.Errorf("peers加入通道失败: %v", err)
		// 	}
		// }
	...
	// } else {
	// 	fmt.Println("通道已经创建且peers已经加入通道")
	// }
...
func InstallAndInstantiateCC(sdk *fabsdk.FabricSDK, info *InitInfo) (*channel.Client, error) {
	...
	//  returns a policy that requires one valid 填组织的msp
